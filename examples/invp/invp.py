...
def dyn(t, X, u):
    ...
    Y = X.copy()
    xc, alpha, xc_dot, alpha_dot = X
    Y[0] = xc                                                        # xc'
    Y[1] = alpha                                                     # alpha'
    Y[2] = 2.2643*alpha -15.8866*xc - 0.0073*alpha_dot + 2.2772*u    # xc''
    Y[3] = 27.8203*alpha -36.6044*xc_dot -0.0896*alpha_dot + 5.247*u # alpha''
    return Y

K = np.array([-44.72, 200.8, -49.77, 27.38])
def controller(X):
    u = -K.dot(X)
    return u
